...
def water_score(input_list):
    ...
    for idx, obj in enumerate(props):
        ...
        bbox = obj['bbox']
        center = obj['centroid']
        count, xpts, ypts = find_maxima(intensity_image, ntol=20)
        if count > 1:
            ...
            label_seg = measure.label(seg_result, connectivity=2)
...
def edgeSmooth(img):
    ...
    # hole fill
    # img_hole = hole_fill(img_filter)
    img_hole = remove_small_holes(img_filter, area_threshold=24, connectivity=2)
    # morphology open
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 9))
    img_hole_open = cv2.morphologyEx((img_hole > 0).astype(np.uint8), cv2.MORPH_OPEN, kernel)

    return (img_hole_open > 0).astype(np.uint8)